...
word_feat_len = 128
WEIGHT_FILE = "./lib/word2vec_weight.model"
TRAIN_FILE = "../aozora_text/files/files_all_tmp.txt"


class StringOperation():
    def __init__(self, load_flag="load"):
        self.word_feat_len = word_feat_len

        self.myW2V = wv.MyWord2Vec()
        if load_flag == "train":
            self.myW2V.train(TRAIN_FILE)
    ...
    def sentens_vec_to_sentens_arr_prob(self, sentens_vec):
        __arr = []
        for value in sentens_vec:
            __prob_word = self.myW2V.vec_to_some_word(
