...
class DataShaping():
    ...
    def train_data_shaping(self, train_sentens_vec_batch, train_sentens):
        if "BOS" in train_sentens:
            train_sentens.remove("BOS")
    ...
    def teach_target_data_shaping(self, teach_target_sentens_vec_batch, teach_sentens):
        if "BOS" in teach_sentens:
            teach_sentens.remove("BOS")
        teach_target_sentens = teach_sentens
        teach_target_sentens.append("。")
        teach_target_sentens_vec = self.str_op.sentens_array_to_vec(
            teach_target_sentens)
    ...
    def make_data(self, word_lists, batch_size, chose_bucket):
        ...
        for j in range(batch_size):
            ...
            while(True):
                train_sentens, teach_sentens = self.select_random_sentens2(
                    __word_lists)
                ...
                if((self.select_bucket(train_sentens, 0) <= bucket_index) and (self.select_bucket(teach_sentens, 1) == bucket_index)):
                    break

            train_sentens_vec_batch = self.train_data_shaping(
                train_sentens_vec_batch, train_sentens, chose_bucket[0])
            teach_sentens_vec_batch = self.teach_data_shaping(
                teach_sentens_vec_batch, teach_sentens, chose_bucket[1])
            teach_target_sentens_vec_batch = self.teach_target_data_shaping(
                teach_target_sentens_vec_batch, teach_sentens, chose_bucket[1])

        train_sentens_vec_batch = np.array(train_sentens_vec_batch)
        teach_sentens_vec_batch = np.array(teach_sentens_vec_batch)
        teach_target_sentens_vec_batch = np.array(
            teach_target_sentens_vec_batch)
    ...
    def make_data_train(self, word_lists, i):
        train_sentens_vec_batch = []
        ...
        train_sentens_vec_batch = self.train_data_shaping(
            train_sentens_vec_batch, train_sentens)
    ...
    def make_data_teach_target(self, word_lists, i):
        teach_sentens_vec_batch = []
        teach_target_sentens_vec_batch = []

        __word_lists = word_lists[::]

        teach_sentens = __word_lists[i]
        teach_sentens = self.str_op.reshape_sentens(teach_sentens)

        print(teach_sentens)
        teach_sentens_vec_batch = self.teach_data_shaping(
            teach_sentens_vec_batch, teach_sentens)
        teach_target_sentens_vec_batch = self.teach_target_data_shaping(
            teach_target_sentens_vec_batch, teach_sentens)
        ...
        return teach_sentens_vec_batch, teach_target_sentens_vec_batch
